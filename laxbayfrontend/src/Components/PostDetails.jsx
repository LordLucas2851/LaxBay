...
export default function PostDetails() {
  ...
  useEffect(() => {
    const fetchPostDetails = async () => {
      try {
        const response = await axios.get(`${API}/store/postdetails/${postId}`);
        setPost(response.data);
        const username = response.data.username;
        const emailRes = await axios.get(`${API}/user/email/${username}`);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <p className="mt-4">
        Contact Email: <a href={`mailto:${email}`} className="text-blue-500">{email}</a>
      </p>
    </div>
  );
}